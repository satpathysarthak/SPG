...
data <- old_df
colnames(data) <- c("file_name","median_1fpkm","me","BH")
data$file_name <- as.character(data$file_name)
data$file_name <- sub("/media/sarthak/gender_prognosis/cox_regression/","",data$file_name)
...
data <- subset(data, select = c("type","model","cancer","median_1","mean_absolute","BH"))
#write.table(data,"/media/sarthak/gender_prognosis/analysis/table/20200813_FDR_filters.txt",quote = FALSE, sep = "\t",row.names=FALSE)
#### Plotting ####

setwd("/media/sarthak/gender_prognosis/plots/20200813_FDR_exploratory")
library(reshape2)
library('tidyverse')
library(ggplot2)
results_long <- melt(data, id.vars=c("cancer","model","type"))
jpeg(filename = "gender_est.jpeg")
results_long1 <- results_long[results_long$model == "est"&results_long$type=="gender",]
results_long1 %>% 
...
results_long1 <- results_long[results_long$model == "int"&results_long$type=="gender",]
results_long1 %>% 
  #filter(type %in% variable) %>%
  ggplot(aes(x=cancer,y=value, fill=variable))+
  geom_bar(stat="identity", position=position_dodge()) + xlab("cancer type") + ylab("frequency")+
...
results_long1 <- results_long[results_long$variable == "BH"&results_long$type=="gender",]
results_long1 %>% 
  #filter(type %in% variable) %>%
