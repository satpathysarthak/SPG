...
library(readr)
library(stringr)
setwd("/media/sarthak/gender_prognosis/cox_regression/FDR_gender")
...
p
##Figure 1 ## 
setwd("/media/sarthak/gender_prognosis/plots/20200902_FDR_gender")
jpeg(filename = "Barplot_S1.jpeg")
p
dev.off()
rm(list = ls())
...
p2a <- p2a +ylim(c(0,1200)) + coord_flip() 
...
p2b <- p2b +ylim(c(0,1200))+coord_flip()
library(gridExtra)
...
##### Common genes #####
rm(list= ls())
setwd("/media/sarthak/gender_prognosis/cox_regression/FDR_gender_name")
