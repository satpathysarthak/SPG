...
mean_abs <- quantile(abs(df_int$Interaction_coef),0.8)
length(which(abs(df_int$Interaction_coef) > mean_abs))
output <- subset(df_int, subset=(abs(df_int$Interaction_coef) > mean_abs)) 
dim(output)
output$FDR<- p.adjust(output$Interaction_Pr...z.., "BH")
length(which(output$FDR < 0.10))
output <- subset(output, subset=(abs(output$FDR) < 0.10))

ML <- subset(data, select=as.character(output$gene))
...
write.table(frame_out, file = output_file, row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t", append = TRUE)
